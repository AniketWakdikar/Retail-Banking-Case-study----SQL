...
use casestudy;
  ...
  CREATE TABLE ADDRESS 
   (	ADDRESS_ID INT(6), 		ADDRESS_LINE1 VARCHAR(50), 		ADDRESS_LINE2 VARCHAR(50), 
		CITY VARCHAR(30), 		STATE VARCHAR(30), 		PINCODE INT(6), 		COUNTRY VARCHAR(30)
   ) ;
...
CREATE TABLE ONLINE_CUSTOMER 
   (	CUSTOMER_ID INT(6), CUSTOMER_FNAME VARCHAR(20), CUSTOMER_LNAME VARCHAR(20), CUSTOMER_EMAIL VARCHAR(30), 
		CUSTOMER_PHONE BIGINT(10), ADDRESS_ID INT(6), CUSTOMER_CREATION_DATE DATE, CUSTOMER_USERNAME VARCHAR(20), 
		CUSTOMER_GENDER CHAR(1)
   );
   
     CREATE TABLE ORDER_HEADER 
   (	ORDER_ID INT(6), 		CUSTOMER_ID INT(6), ORDER_DATE DATE, ORDER_STATUS VARCHAR(10), 
		PAYMENT_MODE VARCHAR(20), PAYMENT_DATE DATE, ORDER_SHIPMENT_DATE DATE, 	SHIPPER_ID INT(6)
   ) ;
   ...
   (	PRODUCT_ID INT(6), PRODUCT_DESC VARCHAR(60), PRODUCT_CLASS_CODE INT(4), PRODUCT_PRICE DECIMAL(12,2), 
		PRODUCT_QUANTITY_AVAIL INT(4), 	LEN INT(5), WIDTH INT(5), HEIGHT INT(5), WEIGHT DECIMAL(10,4)
   );
...
select product_id,product_class_code, product_price,
case
...
select product_id,product_class_code, product_price,product_price+600 change_price from product
where product_class_code=2052
order by product_price+600 desc;


# 2. List the product description, class description and price of all products which are shipped

select product_desc Product_Description, product_class_desc Class_Description, product_price Price, order_status from 
product p join order_items oi 
...
select product_ID, product_class_desc, product_quantity_avail,
CASE
 when product_quantity_avail < 10 Then "Low stock"
 when product_quantity_avail > 10 and product_quantity_avail < 30 Then "In stock"
 Else 'Enough stock'
 END AS stock_status
from product as p join product_class as pc
on p.product_class_code=pc.product_class_code
...
from product as p join product_class as pc
on p.product_class_code=pc.product_class_code
where product_class_desc not in ("Stationery","Clothes","Electronics","Computer");
...
select customer_fname, customer_lname, state, product_class_desc from
online_customer oc join address a
...
join product_class pc
on p.product_class_code=pc.product_class_code
where product_class_desc not in ((select product_class_desc from product_class where product_class_desc="Toys"),
(select product_class_desc from product_class where product_class_desc="Books")) and state<>"Karnataka";
